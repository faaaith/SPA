...
define(['util', 'selector'], function (util, $) {
    ...
    function init() {
        ...
        if (isMobile) {
            ...
            window.onhashchange = function () {
                ...
                }else if ((oldHash == '#details' && newHash == '#type') || (oldHash == '#type' && newHash == '#details')) {
                    newEle.classList.add('slide', 'reverse', 'in');
                    oldEle.classList.add('slide', 'reverse', 'out');
                    oldEle.style.display = 'block';
                    newEle.style.display = 'block';
    ...
    function typeAdd() {
        ...
        if (fatherName === '-1') {             // 添加分类
            var newCate = {
                'id': cate.length?cate[cate.length - 1].id + 1:0,
                'name': name,
                'num': 0,
                'child': []
            };
            ...
            var newChild = {
                'id': childCate.length?childCate[childCate.length - 1].id + 1:0,
                'name': name,
                'child': [],
